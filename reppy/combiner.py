...
def combine_files(input_dir: str, output_path: str, extension: str = None, **kwargs):
        ...
        for f_idx, file in enumerate(list_files(input_dir, extension)):
            ...
            for chunk_idx, chunk in enumerate(read_csv(file.path)):
                if f_idx > 0 and chunk_idx == 0:
                    chunk = chunk[1:]
                rows += len(chunk)
                logger.debug(f"writing file {file.path} chunk: {chunk_idx} to {output_path}")
                writer.writerows(chunk)

    logger.debug(f"writing done {rows} rows")


combine_files('..\data', r"..\data\combined\new.csv")
